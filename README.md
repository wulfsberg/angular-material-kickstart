...
-----------------------
The default `package.json` generated by `ng new` is in fact a little outdated, so update all dependencies by running

    ng update --all
   
Notably, this updates TypeScript to 2.9+.
...
If you're using AOT-compilation, you can add `--locale=da-DK` to your build command, e.g.
`ng serve --aot --locale=da-DK`. (Production build uses AOT by default, and it will likely be default in dev build in
the future, too).
...
### mat-icon
If you use `mat-icon` with Google's [Material Design Icons](https://material.io/icons/), you need to import the icon
font, too, so add a stylesheet link to `index.html`:
